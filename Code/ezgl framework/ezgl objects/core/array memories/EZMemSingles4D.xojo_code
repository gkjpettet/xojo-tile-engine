#tag Class
Protected Class EZMemSingles4D
Inherits MemoryBlock
	...
	#tag Method, Flags = &h1000
		Sub Constructor(lastIndex As integer)
		  ...
		  Super.Constructor((lastIndex + 1) * 16)
	...
	#tag Method, Flags = &h0
		Sub set(idx As integer, x As single, y As single, z As single, w As single)
		  //no bounds checking of idx
		  
		  dim base As integer = idx * 16
		  
		  refPtr.Single(base) = x
		  refPtr.Single(base+4) = y
		  refPtr.Single(base+8) = z
		  refPtr.Single(base+12) = w
		End Sub
	#tag EndMethod
